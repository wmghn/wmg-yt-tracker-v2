...
export default function DirectorStaffPage() {
  ...
  async function handleRoleChange(userId: string, newRole: string) {
    ...
    setUsers((prev) => prev.map((u) => u.id === userId ? { ...u, role: newRole as "MANAGER" | "STAFF" } : u));
    setUpdating(null);
  }
...
function Section({
  title, icon, count, users, updating, onRoleChange, onToggleActive,
}: {
  title: string;
  ...
  return (
    <div className="rounded-xl border bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b px-5 py-4">
        ...
        <span className="ml-1 rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-500">{count}</span>
      </div>

      {users.length === 0 ? (
        <p className="py-8 text-center text-sm text-zinc-400">Chưa có {title.toLowerCase()} nào</p>
      ) : (
                ...
                <tr key={u.id} className="border-b last:border-0 hover:bg-zinc-50/50">
                  ...
                  <td className="px-5 py-3">
                    <select
                      ...
                      className={`rounded-full border px-2 py-0.5 text-xs font-medium cursor-pointer focus:outline-none ${ROLE_STYLE[u.role]}`}
                    >
